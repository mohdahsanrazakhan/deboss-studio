...
- EmailJS also hard-caps total template-variable size at 50KB per send; the notification email's `{{thumbnail}}` is by far the biggest contributor. The app already renders it small and JPEG-compressed, and drops it (with a short text note) instead of letting the whole email fail if it's ever still too big for a particular design; either way, the full thumbnail is always in the paired Google Sheets row. Note that EmailJS's own dashboard "Test Email" button sends a tiny placeholder string for this field, not a real image, so a successful dashboard test doesn't confirm a real submission's actual thumbnail will fit; if a real submission's notification never shows up in EmailJS's Email History at all (not even as a failure), oversized payload is the likely cause.
