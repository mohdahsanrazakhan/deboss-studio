...
`lib/deboss/direction.ts` exports `detectTextDirection(text)`, a first-strong-character scan over `DebossState.text`. Both the engine (`ctx.direction` in `layoutLines`/`buildMask`, and the rich-text measurement DOM below) and the UI (`dir` on `RichTextEditor.tsx`'s root) call it; there is no manual toggle and no hardcoded direction anywhere. Since `state.text` may now contain rich-text markup, every call site runs it through `stripTags()` (`lib/deboss/richtext.ts`) first: tag names/attributes are Latin characters that would otherwise misdetect LTR on genuinely RTL content. This lets the same app render Urdu/Arabic (RTL) and Latin/Devanagari (LTR) text correctly, script-switching live as the user types.

## Rich-text formatting (bold/italic/underline/per-selection size)

`DebossState.text` stays a plain `string`, now optionally containing a small, fixed, closed tag vocabulary (`<b>`, `<i>`, `<u>`, `<span style="font-size:Npx">`) that only `lib/deboss/richtext.ts`'s serializer ever generates. `RichTextEditor.tsx` (a full WYSIWYG editor built on Tiptap, loaded via `next/dynamic({ ssr: false })` to keep its bytes off the tracked First Load JS) replaces the old plain `<textarea>`; its toolbar drives Bold/Italic/Underline toggles and A-/A+ size steppers, gated per font by `FONT_CAPABILITIES` (`constants.ts`) so a font with no real bold/italic face never renders a browser-synthesized ("faux") style.

`engine.ts` has two layout paths, gated on `hasRichRuns(state.text)`. Untagged text stays on the original scalar path (`layoutLines`, one `fillText` per line, unchanged since before this feature) so every existing `GALLERY_EXAMPLES` entry and the default text render byte-identically to before. Tagged text goes through `measureRichLines`: canvas has no native mixed-run text layout API, and hand-rolling a bidi-aware line-breaking algorithm to replace the single-`fillText`-per-line approach (which gets correct Arabic/Urdu shaping "for free" from one call covering a whole line) risks exactly the kind of subtle RTL regression this project treats as a hard rule. Instead, `measureRichLines` renders the same tagged markup into a hidden, singleton DOM container (same lazy-create-and-reuse pattern as `_measureCtx`/`_noiseTile`) and reads back the browser's own bidi-correct, wrap-correct visual positions via `getBoundingClientRect` for each fragment's x/width, then `buildMask` replicates those exact positions with one `fillText` call per word/sub-word fragment, drawing underlines as manually-filled rectangles into the same mask (so they inherit the shared recess/tint/shadow/highlight compositing for free).
