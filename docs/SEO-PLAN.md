...
- ~~**Blog pipeline**~~: done. `/blog` (index) plus one route per post, `/blog/<slug>`, content-driven from `content/blog/*.mdx` (MDX + frontmatter) via `src/lib/blog/posts.ts` (Node `fs`, server-only), rendered server-side with `next-mdx-remote/rsc` (zero added client JS, `remark-gfm` + `rehype-slug`). Adding a post is dropping a file, no route changes needed. Each post gets its own title/description/canonical, `BlogPosting` JSON-LD, and a per-post OG image (`src/app/blog/[slug]/opengraph-image.tsx`, same Satori trick as gallery's). An optional `coverExampleSlug` frontmatter field reuses an existing `GALLERY_EXAMPLES` entry as a real engine-rendered cover (`GalleryPreview`), falling back to a text-only card. Listed in `sitemap.ts`. **Still pending**: real long-form articles — only a placeholder post (`content/blog/hello-world.mdx`) exists so far. Planned topics: letterpress history, Nastaliq vs Naskh, how the canvas effect works (developer audience earns backlinks).
