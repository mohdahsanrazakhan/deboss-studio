...
- [x] Script-agnostic rich-text editor with automatic RTL/LTR direction detection (`detectTextDirection`) and a Urdu default text (یہاں لکھیں)
...
- [x] Fourteen fonts: Noto Nastaliq Urdu (default), Gulzar, Noto Naskh Arabic, Playfair Display, Noto Serif Devanagari, Amiri, Reem Kufi, Reem Kufi Fun, Aref Ruqaa, Lateef, Rakkas, Mirza (Google Fonts), plus Jameel Noori Nastaleeq and its Kasheeda variant (self-hosted from `public/fonts/`); loaded and awaited before first canvas paint
