...
| Change preset deep-link behavior | `src/app/page.tsx` (`resolvePresetId`, `generateMetadata`) → `Studio.tsx` (`initialPresetId` prop) → `useDebossStudio.ts` (the initial-preset `useEffect`, `setPresetInUrl`/`clearPresetFromUrl`) → `app/sitemap.ts` |
